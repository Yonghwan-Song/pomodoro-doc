...
- State Validation
  ...
  - `oauth/start` route에서 authorization url을 만들 때 state을 in-memory Map에 저장 후 나중에 redirect url로 오는 request의 state query string과 비교.
    - 현재 API server는 render.com의 starter plan을 이용 중이고, instance는 하나로 제한됩니다.
    - 그래서, `oauth/callback` route로의 GET request를 받게 되는 instance는
    - 처음에 state을 만들어낸 instance와 항상 동일하기 때문에, redis같은 instance들 간에 공유할 수 있는 공간에 따로 state을 저장할 필요는 없었습니다.
